"use client";

import { motion } from "framer-motion";

const skills = [
  { skill: "React", level: 90 },
  { skill: "React Native", level: 70 },
  { skill: "Ruby", level: 40 },
  { skill: "Ruby on Rails", level: 40 },
  { skill: "JavaScript", level: 90 },
  { skill: "TypeScript", level: 80 },
  { skill: "Tailwind CSS", level: 45 },
  { skill: "React Query / TanStack Query", level: 50 },
  { skill: "Next.js", level: 30 },
  { skill: "Vue.js", level: 30 },
  { skill: "CSS/Sass", level: 95 },
  { skill: "UI/UX Design", level: 85 },
  { skill: "Accessibility", level: 40 },
  { skill: "UX Research", level: 30 },
  { skill: "Figma", level: 30 },
  { skill: "Responsive Design", level: 75 },
  { skill: "Python", level: 20 },
  { skill: "Node.js", level: 25 },
  { skill: "GraphQL", level: 30 },
];

export default function SkillsSection() {
  return (
    <section aria-labelledby="skills-heading" className="relative py-24 px-4">
      <div className="max-w-4xl mx-auto">
        <motion.h2
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="text-3xl font-bold mb-12 text-center"
        >
          Technical Skills & Experience
        </motion.h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {skills.map((item, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
              className="space-y-2"
            >
              <div className="flex justify-between text-sm font-mono">
                <span>{item.skill}</span>
                <span>{item.level / 10} Years of Experience</span>
              </div>
              <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  whileInView={{ width: `${item.level}%` }}
                  viewport={{ once: true }}
                  transition={{ duration: 1, ease: "easeOut" }}
                  className="h-full bg-gradient-to-r from-blue-500 to-green-500"
                />
              </div>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
}
